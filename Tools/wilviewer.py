...
function addFocusedResourceControl(x,y,w,h,frame,state,text){const d=document.createElement('div');d.className='focus-resource-control';d.style.cssText=`left:${x}px;top:${y}px;width:${w}px;height:${h}px`;const im=document.createElement('img');im.src=`/api/image?f=GameInter.wil&i=${frame}&scale=1`;im.alt=`GameInter.wil Frame ${frame}`;im.style.cssText='width:100%;height:100%;image-rendering:pixelated';d.appendChild(im);if(state!=null){const tag=document.createElement('span');tag.textContent=`${text||''} · F${frame}/${state}`;tag.style.cssText='position:absolute;left:0;top:100%;color:#bff7d0;background:rgba(8,12,16,.92);font:10px monospace;white-space:nowrap';d.appendChild(tag);}else if(text){const tag=document.createElement('span');tag.textContent=text;tag.style.cssText='position:absolute;left:0;top:100%;color:#bff7d0;background:rgba(8,12,16,.92);font:10px monospace;white-space:nowrap';d.appendChild(tag);}screen.appendChild(d);}
