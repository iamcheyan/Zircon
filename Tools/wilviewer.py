...
function addInternalControl(r,windows){if(!r.position||r.coordinate_status==='unresolved')return;const pair=r.resource?.frame_pair||[];const w=r.size?.width||20,h=r.size?.height||20;let x=r.position.x,y=r.position.y;const parent=windows.find(q=>q.id===r.window_id);const relative=r.position.coordinate_space==='window-relative';if(relative&&parent){x+=parent.position.x;y+=parent.position.y;}const box=document.createElement('div');box.className=`internal-control ${r.coordinate_status==='outside-window'?'outside-control':''} ${r.size?'':'unresolved-size'}`;box.style.cssText=`left:${x}px;top:${y}px;width:${w}px;height:${h}px`;if(pair.length){const im=document.createElement('img');im.src=`/api/image?f=GameInter.wil&i=${pair[0]}&scale=1`;im.alt=`GameInter.wil Frame ${pair[0]}`;im.style.cssText='width:100%;height:100%;image-rendering:pixelated';box.appendChild(im);}addLabel(box,`${r.window_id} · F${pair.join('/')} · (${x},${y},${w},${h})${relative?' · window-relative':''} · ${r.coordinate_status}${r.size?'':' · size unresolved'}`,'button-label');box.onclick=()=>focusRecord(r.id);screen.appendChild(box);}
