...
function addFocusedWindow(r,analysis){
  ...
  const local=(r.id==='window.horse'?DATA.layout.horse_window_evidence?.paint_order:null)||(r.id==='window.status'?evidenceWindow(r.id)?.paint_state?.paint_order:null)||(r.id==='window.inventory'?evidenceWindow(r.id)?.paint_order:null)||(r.id==='window.quest'?evidenceWindow(r.id)?.paint_order:null);
