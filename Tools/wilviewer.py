...
function addMapMinimapCandidate(l){const r=l.map_ui_evidence?.viewport?.fixed_minimap_widget?.screen_rect||{left:672,top:0,right:800,bottom:128};const frame=document.createElement('div');frame.style.cssText=`position:absolute;left:${r.left}px;top:${r.top}px;width:${r.right-r.left}px;height:${r.bottom-r.top}px;overflow:hidden;background:#08110e;border:2px solid #e8a33d;z-index:24`;const image=document.createElement('img');image.src=`/api/image?f=${encodeURIComponent(MAP_FILE)}&i=${MAP_FRAME}&scale=1`;image.style.cssText='width:128px;height:128px;object-fit:contain;image-rendering:auto;opacity:.92';frame.appendChild(image);const label=document.createElement('div');label.style.cssText='position:absolute;left:4px;top:4px;color:#ffd58a;background:rgba(10,14,18,.88);padding:2px 3px;font:10px monospace;z-index:2';label.textContent=`${MAP_FILE} F${MAP_FRAME} → 128×128`;frame.appendChild(label);screen.appendChild(frame);const note=document.createElement('div');note.style.cssText='position:absolute;left:14px;top:10px;color:#e8a33d;background:rgba(10,14,18,.88);padding:4px 6px;border:1px solid #e8a33d;font:12px monospace;z-index:70';note.textContent=`[固定小地图目标 Rect (${r.left},${r.top})-(${r.right},${r.bottom}) · ${MAP_FILE} F${MAP_FRAME} 仅作资源缩放候选 · 边框/裁剪/标记仍未冒充已确认]`;screen.appendChild(note);}
...
function addLayerLegend(l){const box=document.createElement('div');box.className='layer-legend';const layers=l.draw_order_evidence?.layers||[];box.innerHTML='<b>原版绘制层级候选</b><br>'+layers.map(x=>`<div>${esc(String(x.order).padStart(3,'0'))} · ${esc(x.id)} <span style="color:#9ba7b4">[${esc(x.confidence||'candidate')}]</span></div>`).join('');screen.appendChild(box);addVisibleWindowDispatch(l);}
