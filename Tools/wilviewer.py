...
function save(){localStorage.setItem(key,JSON.stringify({debug:$('#debug').checked,frames:$('#frames').checked,mode:SCREEN_MODE,diffShow:$('#diff-show').checked,diffOpacity:$('#diff-opacity').value,mapRect:$('#map-rect').checked,mapCandidate:$('#map-candidate-layer').checked,layers:$('#layers').checked,mapFile:MAP_FILE,mapFrame:MAP_FRAME}));}
...
function addLayerLegend(l){const box=document.createElement('div');box.className='layer-legend';const layers=l.draw_order_evidence?.layers||[];box.innerHTML='<b>原版绘制层级候选</b><br>'+layers.map(x=>`<div>${esc(String(x.order).padStart(3,'0'))} · ${esc(x.id)} <span style="color:#9ba7b4">[${esc(x.confidence||'candidate')}]</span></div>`).join('');screen.appendChild(box);addVisibleWindowDispatch(l);addHudBarDispatch(l);}
