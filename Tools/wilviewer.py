...
function load(){const s=state();if(typeof s.debug==='boolean')$('#debug').checked=s.debug;if(typeof s.frames==='boolean')$('#frames').checked=s.frames;if(typeof s.mode==='string')SCREEN_MODE=s.mode;if(typeof s.diffShow==='boolean')$('#diff-show').checked=s.diffShow;if(s.diffOpacity!=null)$('#diff-opacity').value=s.diffOpacity;if(typeof s.layers==='boolean')$('#layers').checked=s.layers;$('#mode').value=SCREEN_MODE;screen.classList.toggle('show-layers',$('#layers').checked);$('#mode').onchange=()=>{SCREEN_MODE=$('#mode').value;save();render()};$('#layers').onchange=()=>{screen.classList.toggle('show-layers',$('#layers').checked);save()};$('#diff-show').onchange=()=>{save();render()};$('#diff-opacity').oninput=()=>{save();render()};$('#diff-file').onchange=()=>{const f=$('#diff-file').files?.[0];if(!f)return;const reader=new FileReader();reader.onload=()=>{DIFF_URL=String(reader.result||'');$('#diff-show').checked=true;save();render()};reader.readAsDataURL(f)};$('#debug').onchange=()=>{screen.classList.toggle('debug',$('#debug').checked);save()};$('#frames').onchange=()=>{screen.classList.toggle('no-frames',!$('#frames').checked);save()};$('#reset').onclick=()=>{localStorage.removeItem(key);location.reload()};fetch('/api/ui-layout').then(r=>r.json()).then(d=>{DATA=d;render()}).catch(e=>$('#summary').textContent='读取布局失败：'+e);}
load();
</script></body></html>"""
