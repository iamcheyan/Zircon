...
- [x] 完成多地图地形和大图对象对照（258 map 解析 + 五族 0/1/5/D001/E01 渲染审计 PASS；原版同图截图 A/B 受 P-002 限制）
- [x] 完成全部技能轨迹/帧链对照（逐技能 A/B 表见 `docs/notes/31-技能施法轨迹分类与Godot迁移.md`；代码级，截图 A/B 待 P-002）
- [~] 完成天气、昼夜、局部光照截图对照（代码级 + Vulkan 渲染审计 PASS；原版亮度曲线/粒子截图 A/B 受 P-002 限制未做）
- [x] 完成 UI 窗口和 2x 锚点快照（当前代码 42 项 UI 审计 PASS，`/tmp/ui_test.png`；原版窗口截图 A/B 待 P-002）
- [~] 完成网络状态和场景生命周期回放（静态 NetworkAudit + 在线 S13-S16 实测 PASS；邮件断线重连等真实序列仍待录）
- [ ] 清空所有已确认差异
...
2026-08-08：最终全量回归完成（headless + desktop Vulkan，全部 exit=0 干净退出）：headless `--map-audit`（258 文件/25,856,732 格/186,728 引用）、`--shadow-audit`（110 库 698,766 帧，thinContent=0 longContent=0）、`--action-audit --skip-sound-audit`（TransparencyAudit 312 库 3,189 帧 cornerPollution=0；WeatherAudit 9/9 alphaMismatch=0；LayerOrderAudit；MagicFrameAudit skills=142 originalResourceExceptions=1（GreenSludgeBall impact dir0-only）；SpellTimingAudit）、`--light-audit`、`--player-matrix-audit`（23,552 组合）、`--cursor-audit`、`--network-audit`（NetworkAudit+AnomalyReplay）、`UITestScene --ui-audit`（11 项）全部 PASS；desktop `--render-audit`、`--light-render-audit`、`--weather-render-audit`、`--projectile-audit`（21 样本/198.4px）、`--map-family-render-audit`（5 族 16.4s）全部 PASS，证据 PNG 重生成（时间戳 20:23）。排查结论：此前"PASS 后不退出"是 `--quit-after` 放在 `--` 之后被当作用户参数（引擎永不退出）叠加其他会话遗留后台批处理并发覆盖 DLL 所致，均非代码缺陷；已写入手交接文档。
2026-08-08：修复在线实战 `ObjectDisposedException: Cannot access a disposed object. Object name: 'ZirconClient.Scripts.ObjectRenderer'`（`MirEffectNode._Process` 读已释放目标节点 `Position`）。根因：怪物被 `S.ObjectRemove` 释放后仍播放的一次性特效每帧访问已释放原生节点；原版 `MirEffect.Target` 是托管 `MapObject`，目标移除后效果冻结在最后 `DrawX/DrawY` 播完（不崩溃），全局特效语义相同。修复：`MirEffectNode._Process`/`UpdateRenderLayer`/`CurrentRenderY` 与 `MirProjectileNode._Process` 均用 `IsInstanceValid` 守卫，目标已释放时冻结最后位置并回退格子排序（飞行物按原版 `Target?.CurrentLocation ?? MapTarget` 回退目标格），不再触碰已释放节点。新增确定性回归审计 `--effect-target-free-audit`：锚定特效+飞行物到临时节点→3 帧跟随→`Free()` 目标→继续跑帧，断言特效存活、位置冻结、无异常；实测去掉守卫后该审计即抛 9 次 `ObjectDisposedException`（审计敏感），恢复修复后全部 PASS。在线回归：真实 `ServerCore`+BotRunner 环境进入 map=1 作战 100s，0 次 `ObjectDisposedException`；headless `--action-audit`、`--projectile-audit`（samples=46 travel=200.4px）全部 PASS。
- 2026-08-08：P1 Input/Combat 闭合（2026-08-08 本轮）——`CombatController._Process` 按原版 `MapControl.ProcessInput` 分支顺序重写：顶部自动攻击（875-895，相邻 Chebyshev==1、冷却到、未骑马、无 ElementalHurricane、怪物无宠物主或 Shift）、AutoRun（896-901）先于鼠标分支与底部追击、Shift+左键未选中（904-913）朝鼠标方向攻击、底部追击（1058-1129）按 `MoveTime` 600ms 节拍 `C.Move`、目标死亡保留（D15）、被阻挡 `BestApproachDirection`/原地转向；`_Input` 按 `OnMouseDown` 683-739 的 Shuriken 分支（超 `MagicRange` 提示+`Stop()`、骑马落近战、冷却清目标、可投 `RangeAttack`+`Stop()` 任意距离）。攻击冷却恢复 `UserObject.SetAction` 637-698 公式 `max(800, AttackDelay - AS*ASpeedRate)`（超重/Neutralize x2），移除旧 `max(250,...)` 地板；转向 `SendTurn` 同向不重复发包；施法行走动画期间排队（D20）。验证：`--combat-audit` 静态矩阵 PASS；窗口化 Vulkan 实服 S16（真实 `C.Attack` 发包 gap=1359/1371/1386ms vs 公式 1359ms、`S.ObjectDied` 后死亡目标保留）ext24/25/26 `RESULT combat=True pass=True`。
- 2026-08-08：P1 影子代码级 A/B 复核——`RenderPrimitives.DrawSilhouetteShadow` 与原版 `PlayerObject.DrawShadow2` 数学逐项一致（`x'=(x-l)-(y-t)/2+H/2+ShadowOffSetX`、`y'=(y-t)/2+ShadowOffSetY`，矩阵 `(1,0,-0.5,0.5)`、opacity 0.5）；`PlayerRenderer.DrawShadow` 坐骑分支匹配 `DrawBody` HorseShape（6/7 用外观库+DrawFrame，其余基础 HorseLibrary+HorseFrame 5000 偏移）；无几何椭圆兜底与原版一致。`--shadow-audit` 当前代码重跑 PASS（libraries=110 frames=698766 metadata=5268 metadataUsable=5268 thinContent=0 longContent=0 fallbackTypes=49:1856）。
- 2026-08-08：P2 UI 全套当前代码重跑 PASS（Xvfb :99 + llvmpipe Vulkan 窗口化，`--rendering-method forward_plus`）：`--ui-audit`（UIAudit/HUD/Trade/ItemGrid/InventorySale/ItemAmount/ItemLock/ItemHover/Equipment/BeltLink/Socket/BeltPotion 12 项）+ `--npc-audit --communication-audit --magic-audit --ranking-audit --monster-audit --quest-audit --chat-audit --character-audit --storage-audit --minimap-audit --fortune-audit --fishing-audit --companion-audit --group-audit --guild-audit --gamestore-audit --consignment-audit --currency-audit --help-audit --horse-audit --dungeon-audit --edit-character-audit --auto-potion-audit --group-lfg-audit --config-audit --keybind-audit --window-chrome-audit`（30 组）合计 42 PASS、0 FAIL；截图 `/tmp/ui_test.png`。审计场景在截图后挂起等 Esc，由 timeout 终止属预期，不影响 PASS 完整性。
