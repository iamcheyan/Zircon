...
### Finding 63：完整地图窗口的容器和子控件坐标从主初始化中恢复（2026-08-10）
...
因此，按通用窗口参数的前七个槽位解释，完整地图容器的屏幕矩形候选为
`(107,110)-(691,362)`。这与固定小地图 `(672,0)-(800,128)` 是两条不同的 UI 路径：
小地图使用地图对象的 `owner+0x2C0` 视口；完整地图使用 ID 15 的 GameInter Frame 602
窗口包装器。
...
### Finding 64：完整地图对象确实进入主 UI 生命周期，并拥有独立绘制/命中分发（2026-08-10）
...
这闭合了“完整地图窗口构造 → 主 UI 生命周期 → 子控件绘制/命中”的证据链。当前仍未把
`FMMap.wil` 的具体帧号、地图拖动/缩放字段和地图内容合成调用强行绑定到 Frame 602；这些
属于下一阶段的状态/绘制证据。
...
确认/提示和完整地图 ID 15；没有出现一个可以独立命名为好友/好友列表的窗口构造函数，也没有
