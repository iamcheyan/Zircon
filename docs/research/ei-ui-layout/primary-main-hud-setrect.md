...
| 候选条 | 原版计算 | 机器码位置 | 状态 |
|---|---|---|---|
| 第一资源条（对应 `[this+0xC68]` Rect） | `low16(0x007D9264) / low16(0x007D9262)` | `0x00429876–0x004298B1` | 比例链已确认，HP 语义待运行时命名 |
| 第二资源条（对应 `[this+0xC78]` Rect） | `low16(0x007D9266) / low16(0x007DA113)` | `0x0042996C–0x004299A4` | 比例链已确认，MP 语义待运行时命名 |
| 经验进度 | `0x007DA115 / 0x007DA119` | `0x00429920–0x0042994F` | 比例链已确认 |
| 额外状态进度 | `low16(0x007DA109) / low16(0x007DA11F)` | `0x0042996C–0x004299A4` | 候选，不能擅自命名 |
...
这使“经验条下方的百分比/进度文字”从视觉猜测提升为机器码候选；格式字面量仍需按
GB 编码单独解出，因此现阶段不把它硬命名成某个中文标签。
