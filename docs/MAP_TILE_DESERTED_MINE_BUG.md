...
- 蒙太奇目视（`catalog/lib-frames/previews/02_tiles5c.png`）：15 个抽样帧中
  7 个纯黑（20,21,22,23,24,11954,13234），帧 0 = 水面，其余为土/岩/棕粒纹理。
- 洞穴图 D1423（EI 集合黑帧最多，29697 格）模拟器三模式帧
  `comparisons/sim_D1423__{none,all,midfront}.webp` 目视确认为大面积纯黑洞穴。

→ 与 §10 终局裁决一致：黑帧是官方资源 + 官方地图引用的本来样貌，非渲染 bug。
